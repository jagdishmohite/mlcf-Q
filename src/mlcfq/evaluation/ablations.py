...
def full_model_config(profiles: list[ClientProfile]) -> AblationConfig:
    """The complete, unablated model -- the baseline every ablation is
    compared against.
    """
...
def all_ablations(profiles: list[ClientProfile]) -> list[AblationConfig]:
    """Every ablation condition from paper §7, plus the full model as the
    reference point. The cohort-test ablation isn't included here since
    it's a population-level mechanism, not a per-session scoring
    configuration -- run `mlcfq.cohort.dispersion.compute_cohort_dispersion`
    or skip it, rather than passing it through `rank_profiles`.
    """
    return [
